class Repo
  class Filesystem
    ...
    def find_by(model, criteria)
      path = path_for(model.name.tableize) << criteria[:name]
    ...
    def save(entity)
      directory = path_for(entity.class.name.tableize)
    ...
    def destroy(entity)
      path = path_for(entity.class.name.tableize) << entity.name
    ...
    def all(model)
      Dir.entries(path_for(model.name.tableize)).sort.map do |file|
